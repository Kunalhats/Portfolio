          ...
          <nav className="flex flex-col p-4 space-y-4 text-xl font-semibold text-purple-950">
            {["Home", "About", "Technologies", "Projects", "Contact"].map((text) => (
              <NavLink
                ...
                onClick={toggle}
                className={({ isActive }) =>
                  `block py-2 px-4 rounded transition ${isActive ? 'bg-pink-950 text-white' : 'hover:bg-pink-950 text-white'}`
                }
              >
                {text}
              </NavLink>
          ...
          <div className="flex justify-around p-4 border-t border-gray-200">
            {[
              ...
              <a key={index} href={social.href} target="_blank" rel="noopener noreferrer">
                <svg className="w-6 h-6 fill-current text-blue hover:text-sky-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
